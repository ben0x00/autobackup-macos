...
This project provides a set of scripts designed to perform automated backups and cleanup tasks on macOS using the `rsync` utility.
The primary scripts included are `rsync-backup.sh` and `rsync-cleanup.sh`.
By default, only tracked changes are cloned, hugely reducing transfer time, which is especially useful for slower USB thumb drive use. 
Cleanup script allows folders to be synced by removing files that are no longer in the source folder.
...
ℹ️ These scripts can be run in conjunction with `cron` or `automator`/`Apple Script` to start either on a schedule or as drives are connected to the system.
## Script Descriptions
